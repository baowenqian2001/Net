...
class Root(nn.Module):
    def __init__(self, in_channels, out_channels, kernerl_size, residual):
        super(Root, self).__init__()
        self.conv = nn.Conv2d(in_channels, out_channels, kernel_size=kernerl_size, stride=1, padding=(kernerl_size - 1)//2)
        self.bn = nn.BatchNorm2d(out_channels)
        self.relu = nn.ReLU(inplace=True)
        self.residual = residual 

    def forward(self, *x): # why?
        children = x 
...
class DLA(nn.Module):
    def __init__(self, layers, channels, num_classes=1000, block=BasicBlock, residual_root=False, pool_size=7):
        ...
        self.num_classer = num_classes

        self.pathfy_stem = nn.Sequential(
    ...
    def forward(self, x):
        ...
        x = self.pathff_stem(x)
        for i in range(6):
            x = getattr(self, 'stage_{}'.formate(i))(x)
